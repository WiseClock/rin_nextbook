---
part: 钓鱼
title: 钓竿
updated: '2021-06-30'
---

 |
-|-
首次出现|2015年8月5日
支持指令|`/钓竿`[]()`/鱼竿`

---

!> 提示
本功能还未移植完毕。部分指令或许不能正常工作。

## 简介



---

## 指令

指令|功能
---|---
/钓竿|查看自己当前装备的钓竿属性。
/钓竿 基础效果|查询当前已装备的钓竿的基础属性。
/钓竿 列表|查看当前钓竿商店内售卖的所有钓竿列表。
/钓竿 说明 **`展示号`**|以商店内显示的钓竿编号查询一款钓竿的属性。
/钓竿 说明 **`名称`**|以商店内显示的钓竿名称查询一款钓竿的属性。
/钓竿 购买 **`展示号`**|以商店内显示的钓竿编号购买一款钓竿。
/钓竿 我的 `页码`|查询自己所拥有的所有钓竿列表。
/钓竿 排行|查询所有玩家拥有的钓竿排行榜。
/钓竿 装备 **`编号`**|以钓竿编号装备一根自己拥有的钓竿。
/钓竿 卸下|卸下当前装备的钓竿。
/钓竿 修理|花费一定铜板修理当前钓竿。
/钓竿 石头|显示自己所拥有的各类石头数量。
/钓竿 买精灵石 **`数量`**|购买[]()**`数量`**个精灵石。
/钓竿 强化说明|查看钓竿强化说明（参阅[强化成功率与素材](#强化成功率与素材)）。
/钓竿 强化|强化一次当前已装备的钓竿。
/钓竿 强化 **`编号`**|强化一次当前未装备的钓竿。
/钓竿 附魔说明|查看钓竿附魔说明。
/钓竿 卷轴 `页码`|查看自己所拥有的所有附魔卷轴列表。
/钓竿 附魔 **`卷轴编号`**|使用拥有的卷轴编号对当前已装备钓竿附魔。

---

## 耐久度

在v9.x中，钓竿加上了耐久度系统。每一根钓竿的初始最大耐久度都是100，但最大耐久度会随着[强化](#强化)而增加或降低：每成功强化一次，最大耐久度都会+3，而每失败一次则会降低`4*损失的强化等级`点耐久度。

举例来说，一根没有强化过的钓竿强化到+1时，耐久度会从100升至103。而一根从+7强化失败跌回+0的钓竿会损失`7*4=28`点耐久度。

### 耐久度损耗

耐久度的损耗取决于玩家钓上的物体。物体越大，则损耗的耐久度越多。通常损耗值介于1~3点间。

### 钓竿维修

使钓竿保持一定耐久度与最大耐久度是至关重要的。当一根钓竿的耐久度为0时玩家将不再能使用它钓鱼，而当最大耐久度为0时玩家将不再能强化它。

最大耐久度没有办法手动增加，但使用[指令](#指令)可以把耐久度修满至最大耐久度。

维修钓竿的花费计算方法是：
> 花费=(钓竿估值÷10÷最大耐久度×损耗的耐久度)的向上取整值

所以，相对来说，钓竿估值越便宜、最大耐久度越高，则维修费用越低。

---

## 估值

钓竿估值被运用在多个地方，包含但不限于钓竿排行、维修费用计算与物品交换劳玲费计算等。

当前的钓竿估值计算方式为：
> 估值=(钓竿基础价格×(强化等级^2^÷100+1))的四舍五入值

要注意的是，钓竿基础价格与钓竿在商店的标价并不一致。

---

## 强化

钓竿最高可以被强化至+15，每个强化等级都需要不同的素材，也有不同的成功率。当然，带来的钓鱼收益也不一样。

强化钓竿所需要的精灵石可以用[指令](#指令)购买，也可以在[钓鱼](/fishing/fishing)中获取。

### 强化成功率与素材

当前等级|目标等级|成功率|所需精灵石|所需铜板
---|---|---|---|---
0|1|100%|5个|30文
1|2|100%|5个|30文
2|3|100%|5个|30文
3|4|75%|8个|50文
4|5|75%|8个|50文
5|6|50%|12个|80文
6|7|50%|12个|80文
7|8|50%|12个|80文
8|9|40%|20个|100文
9|10|40%|20个|100文
10|11|40%|20个|100文
11|12|40%|20个|100文
12|13|33%|30个|150文
13|14|33%|30个|150文
14|15|33%|30个|150文

### 强化保护机制

暂空。

### 强化收益

当前等级|钓鱼时间缩减|价格增幅|出货率增益
---|---|---|---
1|15秒||
2|30秒||
3|45秒||
4|75秒|20%|提高20%**D级**及以上稀有度出货率
5|105秒|20%|提高20%**D级**及以上稀有度出货率
6|135秒|20%|提高50%**C级**及以上稀有度出货率
7|165秒|20%|提高20%**C级**及以上稀有度出货率<br/>提高85%**B级**及以上稀有度出货率
8|195秒|50%|提高20%**C级**及以上稀有度出货率<br/>提高85%**B级**及以上稀有度出货率
9|225秒|50%|提高20%**C级**及以上稀有度出货率<br/>提高85%**B级**及以上稀有度出货率
10|255秒|50%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率
11|300秒|50%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率
12|345秒|100%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率
13|390秒|100%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率<br/>提高200%**SS级**及以上稀有度出货率
14|435秒|100%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率<br/>提高200%**SS级**及以上稀有度出货率
15|480秒|200%|提高20%**C级**及以上稀有度出货率<br/>提高50%**B级**及以上稀有度出货率<br/>提高100%**A级**及以上稀有度出货率<br/>提高200%**SS级**及以上稀有度出货率

---

## 附魔

### 附魔成功率

### 附魔保护机制

### 附魔收益